...
mkValidator dat _ ctx = traceIfFalse "The deadline for beneficiary 1 has not been reached" (signedByBeneficiary1 && deadlineReached)
  where
    info :: TxInfo
    ...
    deadlineNotReached = not $ overlaps (from $ deadline dat) (txInfoValidRange info)
